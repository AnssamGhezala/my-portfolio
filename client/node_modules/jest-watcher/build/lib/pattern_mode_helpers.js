...
const printPatternCaret = (exports.printPatternCaret = (pattern, pipe) => {
  const inputText = `${(_chalk || _load_chalk()).default.dim(
    ' pattern \u203A'
  )} ${pattern}`;
...
const printRestoredPatternCaret = (exports.printRestoredPatternCaret = (
  ...
  pipe.write(
    (_ansiEscapes || _load_ansiEscapes()).default.cursorTo(
      (0, (_stringLength || _load_stringLength()).default)(inputText),
      currentUsageRows - 1
    )
  );
  pipe.write(
    (_ansiEscapes || _load_ansiEscapes()).default.cursorRestorePosition
  );
});
